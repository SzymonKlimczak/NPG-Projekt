...
def inicjalizacjaPrzycisku(root,ekran, pole_na_dane):
    def zapisz_email():
        email = pole_na_dane.get()
        with open("Bazy danych\email.txt", "w") as plik:
            plik.write(email)
        messagebox.showinfo("Złote myśli", f"Adres e-mail {email} został zapisany w pliku email.txt.")
        root.destroy()
    przycisk = tk.Button(root, text = "Wyślij", command=zapisz_email)
    przycisk.grid(row = 2)
